...
(function(window, document, Granite, $, SCFConsole) {
        ...
        _.each(that.config.configuration, function(config) {
            if (config.id == "cq:privilegedMembers") {
                ...
                var allowPrivilegedUsers = that.getElementFromModal(
                    "[data-config-id=\"allowPrivilegedMembers\"]").get(0).checked ? true : false;
        ...
        _.each(this.config.configuration, function(config) {
            ...
            if (!_.isEmpty(configEl)) {
                ...
                if (type === "checkbox") {
                    ...
                    var optEls = configEl.find("coral-select-item");
                    if ($.type(val) === "string") {
                        try {
                            val = JSON.parse(val);
                        } catch (e) {
    ...
    var getCheckboxValue = function(conf, $inputEl, selectAllNamespacesEl, that) {
        conf.value = $inputEl.get(0).checked ? true : false;
        if ($inputEl.attr("id") && $inputEl.attr("id") === selectAllNamespacesEl.attr("id")) {
            ...
            if (selectAllNamespacesEl.get(0).checked ? true : false) {
    ...
    FuncConfig.prototype.saveConfig = function() {
        ...
        configs.each(function(i, inputEl) {
            ...
            if (inputType === "checkbox") {
                getCheckboxValue(conf, $inputEl, selectAllNamespacesEl, that);
            } else if (inputType === "coral-select") {
                if (conf.id == "tagNamespaces" && selectAllNamespacesEl.get(0).checked ? true : false) {
                    conf.value = [];
        ...
        console.log(funcConfigs);
        var functionsWrapper = {};
        functionsWrapper.functions = JSON.stringify(funcConfigs);
        functionsWrapper[':operation'] = 'social:validateFunctions';
        functionsWrapper.siteUrl = window.location.href.split('.html')[1];
        var response = $.ajax({
            type: "POST",
            url: 'social/console/components/hbs/editsite/validatefunctions.social.json',
            data: functionsWrapper,
    ...
    FuncConfig.prototype.addListenertoUserListSelect = function(allowUsers) {
        ...
        if (userListEl && userListEl.get(0).checked ? true : false) {
    ...
    FuncConfig.prototype.mutallyLockAllNamespacesAndOthersNS = function() {
        ...
        $an.on("change", function() {
            if ($an.get(0).checked ? true : false) {
                $tn.addClass("scf-is-hidden");
            } else {
                $tn.removeClass("scf-is-hidden");
            }
